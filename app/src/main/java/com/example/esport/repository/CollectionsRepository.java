...
public class CollectionsRepository {
     ...
     */
    public void loadCollectionsFromServer(Callback<Service> callback){
        // get the request
        //APIService request = createRequest();

        // send the request asynchronously, and processing the response in the callback.
        //request.getService().enqueue(callback);
        RetroficManager.getInstance().createRequest().getService().enqueue(callback);
    }
