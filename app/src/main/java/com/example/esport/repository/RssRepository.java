...
public class RssRepository {
     ...
     */
    public void loadRssFromServer(Callback<Rss> callback){
        // get the request
        //APIService request = createRequest();

        // send the request asynchronously, and processing the response in the callback.
        //request.getRss(url).enqueue(callback);
        RetroficManager.getInstance().createRequest().getRss(url).enqueue(callback);
    }
